...
% (optional sanity)
if ~isfolder(base_input_dir)
    error('base_input_dir does not exist: %s', base_input_dir);
end
...
for gi = 1:numel(target_groups)
    ...
    for i = 1:numel(ids)
        sid = ids{i};
        raw_set  = fullfile(base_input_dir, sid, 'eeg', [sid task_name '_eeg.set']);
        clean_set = fullfile(output_dir, sid, [sid '_v2.1_cleaned_final.set']);
        if ~exist(raw_set,'file'), continue; end        % skip if raw missing
        if exist(clean_set,'file'), continue; end       % skip if already cleaned
        subject_list{end+1} = sid; %#ok<AGROW>
        picked = picked + 1;
        if picked >= k_per_group, break; end            % stop after k_per_group
    end
end

% ---- log selected subjects ----
ts = datestr(now,'yyyymmdd_HHMMSS');
logfile = fullfile(logdir, ['subject_list_' ts '.txt']);
fid = fopen(logfile,'w');
fprintf(fid,'%s\n',subject_list{:});
fclose(fid);
fprintf('Selected %d subjects: %s\nLog saved to %s\n', numel(subject_list), strjoin(subject_list, ', '), logfile);
