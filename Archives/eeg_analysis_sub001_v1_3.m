...
% ======================= PARAMETER SETUP ==========================
subject_id = 'sub-001';
output_prefix = [subject_id '_v1.3'];

input_path = ['/Users/bohago/Desktop/neuro analysis/EEG resting-state_Jun 2025/' subject_id '/eeg/'];
input_filename = [subject_id '_task-eyesclosed_eeg.set'];

output_dir = 'outputs';  % Define the folder name first
...
% Master QC log: create fresh if looping or first run
% Initialize the QC CSV file with headers only (if not exist)
...
band_names = fieldnames(band_defs);  % {'theta', 'alpha', 'beta'}
% ==================================================================

% Skip preprocessing if cleaned set already exists
load_cleaned_set = exist(fullfile(output_dir, [output_prefix '_cleaned_final.set']), 'file');
eeglab;

if load_cleaned_set
    EEG = pop_loadset('filename', [output_prefix '_cleaned_final.set'], 'filepath', output_dir);
    ...
    % Save final dataset
    EEG = pop_saveset(EEG, 'filename', [output_prefix '_cleaned_final.set'], 'filepath', output_dir);
end

% Power Spectral Density (PSD) Computation
[psdAll, freqs] = spectopo(EEG.data, 0, EEG.srate, 'plot', 'off', 'freqrange', [2 25]);
writematrix(psdAll, fullfile(output_dir, [output_prefix '_psd_matrix.csv']));

% PSD scalp maps
close all;
specdata = pop_spectopo(EEG, 1, [0 EEG.pnts], 'EEG', 'freqrange',[2 25], 'electrodes','off', 'freqs',[6 10 22]);
saveas(gcf, fullfile(output_dir, [output_prefix '_psd_scalp.png']));
...
% Write to CSV
writetable(T, fullfile(output_dir, [output_prefix '_bandpower_matrix.csv']));
...
log_file = fullfile(output_dir, [output_prefix '_qc_log.txt']);
...
save(fullfile(output_dir, [output_prefix '_summary.mat']), 'bandpower_struct', 'band_defs', 'psdAll', 'freqs', 'artifactICs');

% ===== Append QC to master log file =====
% Append QC data to master .csv log
fid = fopen(qc_file, 'a');  % append mode
fprintf(fid, '%s,%d,%.4f,%.4f,%.4f\n', subject_id, length(artifactICs), mean(bandpower_struct.theta), mean(bandpower_struct.alpha), mean(bandpower_struct.beta));
fclose(fid);
...
saveas(gcf, fullfile(output_dir, [output_prefix '_bandpower.png']));
...
saveas(gcf, fullfile(output_dir, [output_prefix '_topomap_summary.png']));
